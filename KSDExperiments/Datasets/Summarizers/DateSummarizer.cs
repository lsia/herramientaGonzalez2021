...
namespace KSDExperiments.Datasets.Summarizers
{
    class DateSummarizer : IDatasetSummarizer
    {
        ...
        public void Summarize(Dataset dataset)
        {
            ...
            DateTime min_date = dataset.Sessions.Min(s => s.Timestamp);
            DateTime max_date = dataset.Sessions.Max(s => s.Timestamp);
            double interval = (max_date - min_date).TotalSeconds / BUCKETS;
            max_date = min_date.AddSeconds(interval);

            for (int i = 0; i < BUCKETS; i++)
            {
                int count = dataset.Sessions.Where(s => s.Timestamp >= min_date && s.Timestamp < max_date).Count();
                log.Info("  < {0,10} {1,6} {2,6:0.00}%", max_date.ToString("dd/MM/yyyy"), count, 
                    Math.Round(100.0 * count / dataset.Sessions.Length, 2));
