...
namespace KSDExperiments.Datasets
{
    public class Session : IComparable<Session>
    {
        ...
        public static Session CreateDummy(byte[] vks, int session_id = int.MinValue)
        {
            Session retval = new Session(vks, null, null);
        ...
        public Session Split(int start)
        {
            return Split(start, VKs.Length - 1);
        }
        public Session Split(int start, int end)
        {
            ...
            Session retval = new Session(ID, User, Timestamp, UserAgent, vks, hts, fts);
        ...
        public Session ReplaceVKs(Session session)
        {
            return ReplaceVKs(session.VKs);
        }

        public Session(byte[] vks, int[] hts, int[] fts)
            : this(int.MinValue, User.GetUser(int.MinValue), DateTime.Now, "", vks, hts, fts)
        {
        }

        public Session(int id, User user, DateTime date, string user_agent, byte[] vks, int[] hts, int[] fts)
        {
            ID = id;
            User = user;
            Timestamp = date;
        ...
        public Session ToImpostor()
        {
            User impostor = User.GetUser(-666, "IMPOSTOR", DateTime.MinValue, Gender.Unknown);
            return new Session(ID, impostor, Timestamp, UserAgent, VKs, Features[TypingFeature.HT], Features[TypingFeature.FT]);
        }
        ...
        public Session[] SplitByVK(VirtualKeys key)
        {
            ...
            for (int i = 0; i < this.VKs.Length; i++)
                if (this.VKs[i] == (byte)key)
                {
                    if (start < i - 1)
                    {
                        Session split = this.Split(start, i - 1);
                        retval.Add(split);
                    }
            ...
            if (start != VKs.Length)
            {
                Session split = this.Split(start, VKs.Length - 1);
                retval.Add(split);
            }
        ...
        public static Session Join(List<Session> sessions)
        {
            ...
            return new Session(vks, hts, fts);
        }

        public static Session SingleSpace
        {
            get
            {
                return new Session(new byte[1] { (byte)VirtualKeys.VK_SPACE },
                    new int[1],
                    new int[1]
                    );
            }
        ...
        public static Session SinglePeriod
        {
            get
            {
                return new Session(new byte[1] { (byte)VirtualKeys.VK_OEM_PERIOD },
                    new int[1],
                    new int[1]
                    );
            }
        ...
        public Session[] SplitWords()
        {
            ...
            { 
                ...
                {
                    Session word = Split(start_pos, end_pos - 1);
                    retval.Add(word);
                }
        ...
        public double GetAlphanumericAverage(TypingFeature feature)
        {
            ...
            for (int i = 0; i < VKs.Length; i++)
                if (Session.IsAlphanumeric(VKs[i]) && Features[feature][i] >= 0 && Features[feature][i] <= 1500)
                {
                    sum += Features[feature][i];
                    count++;
                }
