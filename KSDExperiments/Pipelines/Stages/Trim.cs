    ...
    public class Trim : PipelineStage
    {
        ...
        void TrimSessionKeystrokesMin(Results results)
        {
            ...
            for (int i = 0; i < results.Datasets.Length; i++)
            {
                ...
                log.Info("  {0} trimmed, {1} remain ({2}%)",
                    dataset.Sessions.Length - sessions.Count,
                    sessions.Count,
                    Math.Round(100.0 * sessions.Count / dataset.Sessions.Length, 2));
        ...
        void TrimUserSessionsMin(Results results)
        {
            ...
            for (int i = 0; i < results.Datasets.Length; i++)
            {
                ...
                var sessions_by_user = dataset.Sessions.GroupBy(s => s.User.UserID);
                ...
                int original_users = dataset.Sessions.Select(s => s.User.UserID).Distinct().Count();
