...
namespace KSDExperiments
{
    static class ListExtensions
    {
        ...
        public static void Feed(this ModelFeeder[] feeders, Session session, bool initial_training)
        {
            for (int i = 0; i < feeders.Length; i++)
                feeders[i].Feed(session, initial_training);
        }

        public static Dictionary<string, KeyValuePair<TypingFeature, Model[]>> Rebuild(this Builder[] builders, Session session)
        {
        ...
        public static Session ChooseImpostor(this Session session, List<Session> candidates)
        {
            Session[] arr = candidates.Where(s => s != session).ToArray();
            if (arr.Length == 0)
        ...
        public static void SplitTrainTest(this Session[] sessions, int training_sessions, out Session[] training, out Session[] testing)
        {
