        ...
        public ModelFeeder CreateFeeder(User user)
        {
            ...
            for (int i = 0; i < factories.Length; i++)
            {
                ...
                foreach (TypingFeature feature in bps)
                {
                    ...
                    else if (model_by_factory[factories[i]].Storage == "SqlServerStorage")
                        tmp = new SqlServerStorage(user, names[i], feature, max_context_size[i], max_ngram_size[i], factories[i]);
                    else if (model_by_factory[factories[i]].Storage == "RedisStorage")
                        tmp = new RedisStorage(user, names[i], feature, max_context_size[i], max_ngram_size[i], factories[i]);
                    else
                    ...
                    */
                    {
                        ...
                        ModelStorage tmp = new MemoryOptimizedStorage(user, names[i], feature, max_context_size[i], max_ngram_size[i], factories[i]);
                        mss.Add(tmp);
                        */
                    }
                }
