...
namespace KSDExperiments.FiniteContexts.Profiles
{
    public class Profile
    {
        ...
        public Session[] Impostors { get; private set; }
        ...
        (
            ...
            Session[] impostors, 
            FiniteContextsReport report = null
        )
        {
            User = user;
            ...
            TrainingSessions = new HashSet<Session>();
        }
        ...
        public HashSet<Session> TrainingSessions { get; private set; }
        public void BuildInitialProfile(Session[] sessions, int training_sessions)
        {
        ...
        public void EvaluateLegitimate(Session[] sessions, int training_sessions, bool retrain)
        {
            for (int i = training_sessions; i < sessions.Length; i++)
            {
                ...
                LegitimateTraining.Add(new KeyValuePair<Session, Dictionary<string, double>>(session, numeric_attribute_values));
        ...
        RNG rng = new RNG();
        Session GetRandomImpostor(User user)
        {
        ...
        public List<KeyValuePair<Session, Dictionary<string, double>>> LegitimateTraining { get; private set; }
            = new List<KeyValuePair<Session, Dictionary<string, double>>>();

        List<Dictionary<string, double>> impostor_numeric_attribute_values = new List<Dictionary<string, double>>();

        public List<KeyValuePair<Session, Dictionary<string, double>>> ImpostorTraining { get; private set; }
            = new List<KeyValuePair<Session, Dictionary<string, double>>>();


        const int MAX_METAMODEL_SESSIONS = 100;
        public List<Session> EvaluateImpostors(int count)
        {
            List<Session> selected_sessions = new List<Session>();
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                ImpostorTraining.Add(new KeyValuePair<Session, Dictionary<string, double>>(Impostors[pos], numeric_attribute_values));
        ...
        public void EvaluateFixedImpostors(Session[] impostor_sessions)
        {
            foreach (var session in impostor_sessions)
            {
                ...
                ImpostorTraining.Add(new KeyValuePair<Session, Dictionary<string, double>>(session, numeric_attribute_values));

                Classifier.AppendTrainingSession(false, numeric_attribute_values);
            }
        }

        Dictionary<string,double> EvaluateUserSession(bool legitimate, bool add_session_vector, Session session)
        {
            if (Report != null)
        ...
        public Authentication Authenticate(Session session, RetrainBehavior retrain = RetrainBehavior.TrainOnLegitimate)
        {
            Dictionary<string, KeyValuePair<TypingFeature, Model[]>> pattern_vectors = null;
            var numeric_attribute_values = CalculateAttributes(session, out pattern_vectors);
        ...
        public Authentication[] AuthenticateWithoutRetrain(Session[] sessions)
        {
        ...
        public void CalculateEERs()
        {
            ...
            if (Report != null)
                foreach (var kv in EERs)
                    Report.OnEERFound(kv.Key, kv.Value.EER, kv.Value.Threshold);
        }
