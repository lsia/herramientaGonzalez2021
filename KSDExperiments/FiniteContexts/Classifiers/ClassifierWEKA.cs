...
namespace KSDExperiments.FiniteContexts.Classifiers
{
    public class ClassifierWEKA : Classifier
    {
        ...
        /*
        void AddSession(ARFF arff, Authentication authentication, string category)
        {
            double[] values = new double[authentication.MethodValues.Count];

            int count = 0;
            foreach (var kv in Parameters.NumericAttributes)
            {
                values[count] = authentication.MethodValues[kv.Key];
                count++;
            }

            arff.AppendData(values);
            arff.AppendCategory(category);
        }
        */
        ...
        {
            public CrossValidationInstanceDetail(Session session, string actual, string predicted)
            {
                Session = session;
                Actual = actual;
                Predicted = predicted;
